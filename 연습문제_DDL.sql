...
ALTER TABLE MEMBER_T DROP MEM_ADDRESS;

-- 4. 회원구분 칼럼의 타입을 VARCHAR2(20 BYTE)으로 수정하시오.
ALTER TABLE MEMBER_T MODIFY MEM_GUBUN VARCHAR2(20 BYTE) CHECK(MEM_GUBUN IN('REGULAR','SOCIAL'));

-- 5. 회원패스워드 칼럼의 이름을 MEM_PWD로 수정하시오.
ALTER TABLE MEMBER_T RENAME COLUMN MEM_PW TO MEM_PWD;

-- 6. 회원번호 칼럼에 기본키(PK_MEMBER)를 설정하시오.
ALTER TABLE MEMBER_T MODIFY MEM_NO NUMBER PRIMARY KEY;
...
CREATE TABLE BOARD_T (
    ...
    CONSTRAINT      FK_BOARD_MEMBER     FOREIGN KEY(BOARD_NO)  REFERENCES MEMBER_T(MEM_NO) ON DELETE CASCADE
    );
COMMIT;
...
ALTER TABLE BOARD_T MODIFY TITLE NULL;

-- 10. 글번호 칼럼에 기본키(PK_BOARD)를 설정하시오.
ALTER TABLE BOARD_T MODIFY BOARD_NO NUMBER PRIMARY KEY;

-- 11. 작성자 칼럼에 MEMBER_T 테이블의 회원아이디 MEM_NO 를 참조하는 FK_BOARD_MEMBER 외래키를 설정하시오.
