...
-- 2) 오름차순 결과에 행 번호(ROWNUM)를 붙인다.
-- 3) 행 번호가 2이 데이터를 조회한다.
SELECT B.*
...
-- 1) HIRE_DATE의 오름차순 정렬을 하고 행 번호(ROW_NUMBER 함수)를 붙인다.
...
CREATE TABLE 과장명단(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY)
 AS (SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY FROM EMPLOYEE_T WHERE 1 = 2);
 
INSERT INTO 과장명단(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY)
(SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY FROM EMPLOYEE_T WHERE POSITION = '과장');
