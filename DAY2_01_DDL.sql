...
CREATE TABLE CUSTOMER_T (
    ...
    CUST_AGE NUMBER(3) CHECK(CUST_AGE BETWEEN 0 AND 120),
                    --CHECK 제약조건 CHECK(CUST_AGE BETWEEN 0 AND 120)도 가능
...
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(30BYTE)    UNIQUE; 
...
ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR(6 BYTE) NOT NULL CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE');
                                                --CHECK(GRADE IN('VIP', 'GOLD', 'SILVER', 'BRONZE'));

--6. CUSTOMER_T의 고객명(CUST_NAME) 칼럼의 필수 제약조건을 없애시오.

ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --NULL 을 적어줘야함
