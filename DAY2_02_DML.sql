...
CREATE SEQUENCE DEPT_SEQ
    ...
    NOMINVALUE      --하한선 없음
    NOCYCLE         --번호 순환 없음
    CACHE 20        --20개의 번호를 미리 생성 (번호를 미리 생성하지 않는 NOCACHE 옵션을 앞으로 사용함)
...
CREATE TABLE EMPLOYEE_T (
    ...
    CONSTRAINT  FK_DEPARTMENT_EMPLOYEE FOREIGN KEY(DEPART) REFERENCES DEPARTMENT_T(DEPT_NO) ON DELETE SET NULL
    );
