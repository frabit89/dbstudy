...
SELECT EMPLOYEE_ID, LAST_NAME, NVL2(COMMISSION_PCT, COMMISSION_PCT * SALARY, 0)
FROM EMPLOYEES
...
SELECT LAST_NAME, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS 근무개월수
FROM EMPLOYEES;
...
SELECT PHONE_NUMBER
    , CASE
      WHEN PHONE_NUMBER = 011 THEN 'MOBILE'
      WHEN PHONE_NUMBER = 515 THEN 'EAST'
      WHEN PHONE_NUMBER = 590 THEN 'WEST'
      WHEN PHONE_NUMBER = 603 THEN 'SOUTH'
      WHEN PHONE_NUMBER = 650 THEN 'NORTH'
      ELSE 'Unknown'
      END AS 전화번호
FROM EMPLOYEES;
*/
